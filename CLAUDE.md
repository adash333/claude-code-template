...
ユーザーとのやり取りを後から振り返れるようにするための記録領域:

- **自動保存**: `Stop` フック(`.claude/hooks/save_prompt_log.py`)が Claude の応答完了ごとに `docs/prompt/YYYY-MM-DD.md` へ追記する
- **手動保存**: 特定のやり取りをトピック別ファイルとして残したい場合は、`save-prompt-log` スキルを呼び出して `docs/prompt/YYYY-MM-DD-{トピック}.md` 形式で保存する
- **目的**: 重要な意思決定・設計議論・方針合意を後から検索可能にする
